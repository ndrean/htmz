...
export const options = {
  scenarios: {
    jwt_extreme: {
      ...
      startVUs: 0,
      stages: [
        { duration: "10s", target: 2500 },  // Ramp to 2.5K VUs
        { duration: "10s", target: 5000 },  // Ramp to 5K VUs
        { duration: "10s", target: 7500 },  // Ramp to 7.5K VUs
        { duration: "10s", target: 10000 }, // Ramp to 10K VUs
        { duration: "10s", target: 12500 }, // Ramp to 12.5K VUs
        { duration: "20s", target: 12500 }, // Hold at 12.5K VUs
...
export default function () {
  ...
  // Minimal pause for maximum throughput
  sleep(0.1);
}
...
export function handleSummary(data) {
  ...
  console.log(`
=== JWT EXTREME STRESS TEST RESULTS (C ALLOCATOR) ===
...
🎯 C ALLOCATOR BENEFITS:
- Optimized memory allocation for high frequency operations
- Better performance under extreme load
- Stateless JWT architecture with zero memory growth
