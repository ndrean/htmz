...
fn handleCartOperation(r: zap.Request, operation: []const u8) void {
    ...
    const prefix = if (std.mem.eql(u8, operation, "add")) "/api/cart/add/"
                  else if (std.mem.eql(u8, operation, "increase")) "/api/cart/increase-quantity/"
                  else if (std.mem.eql(u8, operation, "decrease")) "/api/cart/decrease-quantity/"
                  else if (std.mem.eql(u8, operation, "remove")) "/api/cart/remove/"
                  else {
                      r.setStatus(.bad_request);
                      return;
                  };
...
fn on_request_jwt(r: zap.Request) !void {
    ...
    if (std.mem.eql(u8, path, "/")) {
        if (method == .GET) {
            ...
            if (user_jwt == null) {
                new_user_id = std.fmt.allocPrint(allocator, "user_{d}_{d}", .{
                    std.time.timestamp(),
        ...
        if (method == .GET) {
            ...
            for (items, 0..) |item, i| {
                const item_div = std.fmt.allocPrint(allocator,
                    "<div class=\"bg-white rounded-lg p-4 shadow-md flex justify-between items-center transition-transform transform hover:scale-[1.02] cursor-pointer\" hx-get=\"/api/item-details/{d}\" hx-target=\"#item-details-card\" hx-swap=\"innerHTML\">" ++
                    "<div><span class=\"text-lg font-semibold text-gray-900\">{s}</span><span class=\"text-sm text-gray-500 ml-2\">${d:.2}</span></div>" ++
                    "<button class=\"px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-full hover:bg-blue-600 transition-colors\" hx-post=\"/api/cart/add/{d}\" hx-swap=\"none\">Add to Cart</button></div>",
                    .{ i, item.name, item.price, i }
                ) catch {
                    r.setStatus(.internal_server_error);
                    return;
                };
        ...
        if (method == .GET) {
            ...
            // Build cart HTML with actual items
            var cart_html: std.ArrayList(u8) = .empty;
            defer cart_html.deinit(allocator);

            for (payload.cart) |item| {
                const item_html = std.fmt.allocPrint(allocator,
                    "<div class=\"flex justify-between items-center p-4 border-b\">" ++
                    "<div><span class=\"font-semibold\">{s}</span><br><span class=\"text-sm text-gray-500\">${d:.2}</span></div>" ++
                    "<div class=\"flex items-center space-x-2\">" ++
                    "<button class=\"px-2 py-1 bg-red-500 text-white rounded\" hx-post=\"/api/cart/decrease-quantity/{d}\" hx-swap=\"none\">-</button>" ++
                    "<span class=\"px-3 py-1 bg-gray-100 rounded\">{d}</span>" ++
                    "<button class=\"px-2 py-1 bg-green-500 text-white rounded\" hx-post=\"/api/cart/increase-quantity/{d}\" hx-swap=\"none\">+</button>" ++
                    "<button class=\"px-2 py-1 bg-red-600 text-white rounded ml-2\" hx-delete=\"/api/cart/remove/{d}\" hx-swap=\"none\">Remove</button>" ++
                    "</div></div>",
                    .{ item.name, item.price, item.id, item.quantity, item.id, item.id }
                ) catch {
                    r.setStatus(.internal_server_error);
                    return;
                };
        ...
        if (method == .GET) {
            ...
            const allocator = std.heap.c_allocator;
            const item_html = std.fmt.allocPrint(allocator,
                "<div class=\"text-center\"><h3 class=\"text-2xl font-bold text-gray-800 mb-4\">{s}</h3>" ++
                "<div class=\"w-24 h-24 bg-gray-200 rounded-full mx-auto mb-4 flex items-center justify-center\">" ++
                "<svg class=\"w-12 h-12 text-gray-400\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\">" ++
                "<path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z\"></path></svg></div>" ++
                "<div class=\"bg-blue-50 rounded-lg p-6 mb-6\"><p class=\"text-3xl font-bold text-blue-600\">${d:.2}</p>" ++
                "<p class=\"text-gray-600 mt-2\">per unit</p></div>" ++
