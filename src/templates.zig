...
pub const grocery_item_template =
    \\<div class="bg-white rounded-lg p-4 shadow-md flex justify-between items-center transition-transform transform hover:scale-[1.02] cursor-pointer" hx-get="/api/item-details/{d}" hx-target="#item-details-card" hx-swap="innerHTML"><div><span class="text-lg font-semibold text-gray-900">{s}</span><span class="text-sm text-gray-500 ml-2">${d:.2}</span></div><button class="px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-full hover:bg-blue-600 transition-colors cursor-pointer" hx-post="/api/cart/add/{d}" hx-swap="none" onclick="event.stopPropagation()">Add to Cart</button></div>
;

pub const cart_item_template =
    \\<div id="cart-item-{d}" class="flex justify-between items-center p-4 border-b"><div><span class="font-semibold">{s}</span><br><span class="text-sm text-gray-500">${d:.2}</span></div><div class="flex items-center space-x-2"><button class="px-2 py-1 bg-red-500 text-white rounded cursor-pointer" hx-post="/api/cart/decrease-quantity/{d}" hx-target="#qty-{d}" hx-swap="innerHTML" hx-on="htmx:afterRequest: htmx.trigger(document.body, 'cartUpdate')">-</button><span id="qty-{d}" class="px-3 py-1 bg-gray-100 rounded">{d}</span><button class="px-2 py-1 bg-green-500 text-white rounded" hx-post="/api/cart/increase-quantity/{d}" hx-target="#qty-{d}" hx-swap="innerHTML" hx-on="htmx:afterRequest: htmx.trigger(document.body, 'cartUpdate')">+</button><button class="px-2 py-1 bg-red-600 text-white rounded ml-2 cursor-pointer" hx-delete="/api/cart/remove/{d}" hx-target="#cart-item-{d}" hx-swap="outerHTML" hx-on="htmx:afterRequest: htmx.trigger(document.body, 'cartUpdate')">Remove</button></div></div>
;
